...
- ![Feature](https://img.shields.io/badge/Feature-10B981?style=flat-square) **面板内编辑、保存即生效**：点击前置规则 / 追加规则 / 规则集即可在面板内折叠展开编辑区，编辑区限高、规则多时上下滚动；关闭智能引号等自动替换，保存（`⌘S`）后自动重载，只改注释或内容未变时不重载。
