...
- ![Feature](https://img.shields.io/badge/Feature-10B981?style=flat-square) **面板关闭时继续记录曲线**：设置页新增开关，**默认关闭**。状态栏为「仅图标」时面板一关流量流就会停掉，曲线因此留下空档；开启此项后面板关闭期间也继续记录，曲线保持连续。代价是应用不再完全空闲（每秒醒一次），所以默认不开。状态栏切到「图标 + 速率」本来就会让流量流保持开启，那条路无需此开关。
